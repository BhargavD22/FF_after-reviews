...
try:
    ...
    st.error(f"❌ Error fetching data from CSV: {e}")
    st.stop()
...
try:
    # Prepare DataFrame
    forecast_for_csv = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].copy()

    # Convert datetime → date (optional but clean)
    forecast_for_csv['ds'] = forecast_for_csv['ds'].dt.date

    # Optional: Keep lowercase (recommended for consistency)
    forecast_for_csv.columns = ['ds', 'yhat', 'yhat_lower', 'yhat_upper']
