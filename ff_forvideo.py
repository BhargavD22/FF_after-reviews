...
with tab3:
    ...
    st.markdown("---")
    st.markdown("### 📊 Comparative Deep Dive")
    st.markdown("A side-by-side comparison of historical and forecasted trends to help with strategic planning.")

    # Get last 3 months historical and first 3 months forecasted for a specific comparison
    last_hist_df = df.tail(90)
    first_fore_df = forecast_df.head(90)
    
    # Calculate key metrics for comparison
    last_hist_total = last_hist_df['y'].sum()
    last_hist_avg_daily = last_hist_df['y'].mean()
    first_fore_total = first_fore_df[forecast_col].sum() if not first_fore_df.empty else 0.0
    first_fore_avg_daily = first_fore_df[forecast_col].mean() if not first_fore_df.empty else 0.0
    
    # Create a DataFrame for comparison table
    compare_data = {
        'Metric': ['Total Revenue (last/first 3 months)', 'Avg. Daily Revenue (last/first 3 months)'],
        'Historical': [f"${last_hist_total:,.2f}", f"${last_hist_avg_daily:,.2f}"],
        'Forecasted': [f"${first_fore_total:,.2f}", f"${first_fore_avg_daily:,.2f}"]
    }
    compare_df = pd.DataFrame(compare_data)
    st.table(compare_df)

    # Plot for visual comparison
    monthly_rev_hist = df.set_index('ds').resample('M')['y'].sum().tail(6)
    monthly_rev_fore = forecast_df.set_index('ds').resample('M')[forecast_col].sum().head(6)

    fig_comp = go.Figure()
    fig_comp.add_trace(go.Bar(
        x=monthly_rev_hist.index,
        y=monthly_rev_hist.values,
        name='Historical Monthly Revenue',
        marker_color='#0b6ef6'
    ))
    fig_comp.add_trace(go.Bar(
        x=monthly_rev_fore.index,
        y=monthly_rev_fore.values,
        name='Forecasted Monthly Revenue',
        marker_color='#1f9d55'
    ))

    fig_comp.update_layout(
        title="Monthly Revenue: Last 6 Historical vs. First 6 Forecasted",
        xaxis_title="Date",
